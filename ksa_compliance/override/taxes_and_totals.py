...
class calculate_taxes_and_totals:
    def __init__(self, doc: Document):
        ...
        get_round_off_applicable_accounts(
            self.doc.company, frappe.flags.round_off_applicable_accounts
        )
        self.calculate()
    ...
    def determine_exclusive_rate(self):
        ...
        for item in self._items:
            ...
            if (
                ...
                item.net_amount = flt(
                    amount / (1 + cumulated_tax_fraction), item.precision("net_amount")
                )
            ...
            for i, tax in enumerate(self.doc.get("taxes")):
                ...
                if i == 0:
                    ...
                    tax.grand_total_for_current_item = flt(
                        self.doc.get("taxes")[i - 1].grand_total_for_current_item
                        + current_tax_amount
                    )
    ...
    def set_cumulative_total(self, row_idx, tax):
        ...
        if row_idx == 0:
            ...
            tax.total = flt(
                self.doc.get("taxes")[row_idx - 1].total + tax_amount, tax.precision("total")
            )
    ...
    def round_off_base_values(self, tax):
        ...
        if tax.account_head in frappe.flags.round_off_applicable_accounts:
            ...
            tax.base_tax_amount_after_discount_amount = round(
    ...
    def adjust_grand_total_for_inclusive_tax(self):
        ...
        if self.doc.get("taxes") and any(
            cint(t.included_in_print_rate) for t in self.doc.get("taxes")
        ):
        ...
        if not self.doc.get("is_consolidated"):
            for tax in self.doc.get("taxes"):
                if not tax.get("dont_recompute_tax"):
                    tax.item_wise_tax_detail = json.dumps(
                        tax.item_wise_tax_detail, separators=(",", ":")
                    )
        ...
        if self.doc.discount_amount:
            ...
            if total_for_discount_amount:
                ...
                for i, item in enumerate(self._items):
                    ...
                    item.net_amount = flt(
                        item.net_amount - distributed_amount, item.precision("net_amount")
                    )
        ...
        if self.doc.apply_discount_on == "Net Total":
            ...
            for tax in self.doc.get("taxes"):
                if tax.charge_type in ["Actual", "On Item Quantity"]:
                    tax_amount = self.get_tax_amount_if_for_valuation_or_deduction(
                        tax.tax_amount, tax
                    )
                    actual_taxes_dict.setdefault(tax.idx, tax_amount)
                elif tax.row_id in actual_taxes_dict:
                    actual_tax_amount = (
                        flt(actual_taxes_dict.get(tax.row_id, 0)) * flt(tax.rate) / 100
                    )
                    actual_taxes_dict.setdefault(tax.idx, actual_tax_amount)

            return flt(
                self.doc.grand_total - sum(actual_taxes_dict.values()),
    ...
    def calculate_total_advance(self):
        if not self.doc.docstatus.is_cancelled():
            ...
            self.doc.total_advance = flt(
                total_allocated_amount, self.doc.precision("total_advance")
            )
    ...
    def is_internal_invoice(self):
        ...
        if (
            ...
            and self.doc.is_internal_transfer()
        ):
            return True
    ...
    def calculate_change_amount(self):
        ...
        if (
            ...
            self.doc.base_change_amount = flt(
                self.doc.base_paid_amount - base_grand_total,
