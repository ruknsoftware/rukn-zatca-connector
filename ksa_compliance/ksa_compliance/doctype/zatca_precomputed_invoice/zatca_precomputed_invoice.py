...
class ZATCAPrecomputedInvoice(Document):
    ...
    def for_invoice(invoice_id: str) -> Optional["ZATCAPrecomputedInvoice"]:
        prepared_id = frappe.db.exists(
            {"doctype": "ZATCA Precomputed Invoice", "sales_invoice": invoice_id}
        )
        if not prepared_id:
            return None

        return cast(
            "ZATCAPrecomputedInvoice", frappe.get_doc("ZATCA Precomputed Invoice", prepared_id)
        )

    def on_trash(self) -> None:
        frappe.throw(
            msg=_("You cannot Delete a configured ZATCA Precomputed Invoice"),
