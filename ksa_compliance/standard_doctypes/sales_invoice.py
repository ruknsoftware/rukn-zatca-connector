...
def validate_sales_invoice(self: SalesInvoice | POSInvoice, method) -> None:
    ...
    if is_phase_2_enabled_for_company:
        ...
        is_advance_invoice = is_advance_payment_invoice(self, settings)
        valid_advance_payment_invoice = is_valid_advance_payment_invoice(is_advance_invoice, self)
        ...
        if advance_payments:
            if self.doctype == 'POS Invoice':
                ...
                for advance_payment in advance_payments:
                    ...
                    advance_payment_invoice_doc = frappe.get_doc("Sales Invoice", advance_payment.advance_payment_invoice)
                    item = advance_payment_invoice_doc.items[0]
                    tax_percent = abs(item.tax_rate or 0.0)
                    tax_amount = calculate_advance_payment_tax_amount(advance_payment_invoice, advance_payment_invoice_doc)
...
def is_valid_advance_payment_invoice(is_advance_invoice, self) -> bool:
