...
class ValidationDetails:
    ...
    def from_json(j: dict) -> "ValidationDetails":
        return ValidationDetails(
            j["isValid"], j["isValidQr"], j["isValidSignature"], j["errors"], j["warnings"]
        )
...
def check_pdfa3b_support_or_throw(zatca_cli_path: str, java_home: Optional[str]) -> None:
    ...
    fthrow(
        ft(
            "Please update ZATCA CLI to $version or later to support PDF/A-3b generation",
            version="2.5.0",
        )
    )
...
def setup(override_cli_download_url: str | None, override_jre_download_url: str | None):
    ...
    def progress_callback(description: str, percent: float):
        frappe.publish_progress(
            title=ft("Setting up CLI"), percent=percent, description=description
        )
    ...
    try:
        ...
        if not os.path.isfile(zatca_bin):
            fthrow(
                ft("Could not find $zatca_bin after extracting ZATCA archive", zatca_bin=zatca_bin)
            )
...
def sign_invoice(
    ...
    return SigningResult(
        signed_invoice, signed_invoice_path, result.data["hash"], result.data["qrCode"]
    )
...
def run_command(zatca_cli_path: str, args: List[str], java_home: Optional[str]) -> ZatcaResult:
    ...
    if proc.returncode != 0:
        return ZatcaResult(
            is_success=False, msg=result["msg"], errors=result.get("errors", []), data=None
        )
